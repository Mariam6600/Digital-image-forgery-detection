"""
Training pipeline for Methodology 1 (Dual-Input RGB + ELA, ResNet50V2).

Two stages, matching the original experiment:

  1. Head training  (`train_head`)   — both backbones frozen, only the fusion +
     dense head learns. This is the recommended, reproducible stage and is
     what produced the reported 87.88% accuracy / 0.9468 AUC.

  2. "Careful Fine-Tuning Stage 1" (`fine_tune`) — unfreezes only the last
     residual block ('conv5_block3') of *both* backbones with a very low
     learning rate (5e-6). In the original experiment this stage **overfit**
...
from .model_architecture import build_robust_dual_input_model
...
def train_head(model, train_dataset, validation_dataset, epochs: int = config.EPOCHS):
    """Train only the fusion + classification head (both backbones frozen)."""
    class_weights = compute_class_weights("train")
    callbacks = get_head_training_callbacks()

    print(f"Training head for up to {epochs} epochs "
          f"(checkpoint monitors {config.CHECKPOINT_MONITOR}, EarlyStopping monitors {config.EARLY_STOPPING_MONITOR})...")
    history = model.fit(
        train_dataset,
...
def unfreeze_for_fine_tuning(model, layer_name_suffix: str = config.FINE_TUNE_LAYER_SUFFIX):
    """Unfreeze only the last residual block ('conv5_block3' by default) of
    *both* RGB and ELA backbones, keeping everything else frozen."""
    total_unfrozen = 0
    for base_name in ("resnet50v2_rgb_base", "resnet50v2_ela_base"):
    ...
    if total_unfrozen == 0:
        print("WARNING: no layers were unfrozen — check layer_name_suffix against the backbone's layer names.")
    return model


def fine_tune(model, train_dataset, validation_dataset, epochs: int = config.FINE_TUNE_EPOCHS):
    """Optional 'Careful Fine-Tuning Stage 1'. NOTE: in the original experiment
    this caused overfitting — the head-only model is the one actually reported
    / deployed. Kept here so the full pipeline remains reproducible and
    inspectable."""
    import tensorflow as tf
    ...
    try:
        optimizer = tf.keras.optimizers.AdamW(
            learning_rate=config.FINE_TUNE_LEARNING_RATE, weight_decay=config.FINE_TUNE_WEIGHT_DECAY
        )
    except AttributeError:
...
def main():
    parser = argparse.ArgumentParser(description="Train the dual-input (RGB+ELA) ResNet50V2 forgery detector.")
