"""
Evaluation utilities for Methodology 1 (Dual-Input RGB+ELA ResNet50V2).

Run standalone with:  python -m src.evaluation --model-path models/dual_input_resnet50v2_head/best_dual_model_robust_head.keras
"""
import argparse
import json
import os
...
def evaluate(model, validation_dataset, class_names=config.CLASS_NAMES, results_dir=None):
    """Evaluate a trained dual-input model on the validation set.
...
def _save_confusion_matrix_plot(cm, class_names, save_path):
    ...
    plt.title("Confusion Matrix — Methodology 1 (Dual-Input RGB+ELA ResNet50V2)")
    plt.ylabel("Actual")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate a trained dual-input ResNet50V2 checkpoint.")
